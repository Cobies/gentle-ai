...
STATUS derives one immutable live Git snapshot, then loads and validates the selected authority states, compact recovery graph, and receipts once. Valid terminal leaves are projected against that snapshot in memory. Git subprocess count therefore does not grow with terminal history; authority inventory and comparison remain linear CPU/filesystem work.
