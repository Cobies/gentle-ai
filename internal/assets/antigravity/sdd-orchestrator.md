...
3. **Invoke or define the phase subagent**:
   ...
   - **Fallback route**: If static invocation fails or the subagent is not found/uninitialized in the registry, call `define_subagent` to register the subagent dynamically with its system prompt and role-hardened tool permissions before invoking it with `invoke_subagent`.
   - Tool scopes: Set `enable_mcp_tools: true` so phase agents can use configured MCP tools such as Engram. To enforce tool hardening, set `enable_subagent_tools: false` for all subagents, and set `enable_write_tools: false` for all read-only roles (including `sdd-explore`, `review-*` / `review-refuter`, and `jd-judge-*` / `jd-judge-a` / `jd-judge-b`).
...
5. **Synthesize and Fallback Persistence**: Once you receive the message from the subagent containing its final result, read the child result, update DAG/state when applicable, summarize only decisions/outcomes/risks, and ask for approval when interactive mode or review workload guards require it.
   - **Fallback persistence guarantee**: If a subagent completes without persisting directly to Engram, the orchestrator MUST inspect the return envelope and execute fallback persistence via `mem_save` under `sdd/{change-name}/{artifact-type}` (type: `architecture`).
...
- Dynamic subagents are defined with `enable_mcp_tools: true` to perform direct Engram persistence (`mem_save`).
- Fallback persistence guarantee: If a subagent completes without persisting directly to Engram, the orchestrator MUST inspect the return envelope and execute fallback persistence via `mem_save` under `sdd/{change-name}/{artifact-type}` (type: `architecture`).
