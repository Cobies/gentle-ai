...
Use Kimi's `kimi_cli.tools.multiagent:Task` with a bounded general worker for delegated-direct work.
...
Before invoking any Kimi worker via `multiagent:Task`, check your in-session launch log:
...
- Keep one writer; workers return a short handoff with observed outcomes and checks. Preserve unrelated working-tree changes.

### State and Conventions

Convention files live under `~/.config/agents/skills/_shared/` (global) or `.agent/skills/_shared/` (workspace): `engram-convention.md`, `persistence-contract.md`, `skill-resolver.md`.
