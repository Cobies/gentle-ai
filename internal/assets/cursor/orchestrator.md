...
Delegate bounded work to Cursor native sub-agents.
...
Use Cursor native sub-agent delegation for bounded ODD work. Each subagent runs in its own isolated context window; pass the task scope, relevant context, authorized edit surfaces, skill paths, and verification commands explicitly. Collect its result and read back any claimed changes before reporting them.
...
Use a Cursor native bounded worker for delegated-direct work.
...
Pass the authorized edit surfaces, acceptance criteria, configured TDD mode and exact runner, applicable verification commands, and relevant prior context explicitly to bounded implementation workers. Workers preserve unrelated working-tree changes and return observed outcomes and checks. Keep one writer.

### State and Conventions

Convention files under `~/.cursor/skills/_shared/` (global) or `.agent/skills/_shared/` (workspace): `engram-convention.md`, `persistence-contract.md`.
