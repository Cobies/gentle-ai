...
3. **Invoke the static phase subagent**: Call `invoke_subagent` directly using its pre-registered static agent name (`sdd-explore`, `sdd-propose`, `sdd-spec`, `sdd-design`, `sdd-tasks`, `sdd-apply`, `sdd-verify`, `sdd-archive`, `sdd-onboard`, `review-*`, `jd-judge-*`). If the static subagent definition is missing from the registry, you MUST **fail closed**: do NOT call `define_subagent` and do NOT run the phase inline; report `status: blocked (missing static subagent: {name})` immediately. Set `enable_mcp_tools: true` so phase agents can use configured MCP tools such as Engram. To enforce tool hardening, set `enable_subagent_tools: false` for all subagents, and set `enable_write_tools: false` for all read-only roles (including `sdd-explore`, `review-*` / `review-refuter`, and `jd-judge-*` / `jd-judge-a` / `jd-judge-b`).
4. **Invoke the phase subagent**: call `invoke_subagent` with the pre-registered subagent name and a compact task containing approved scope, artifact references, constraints, validation expectations, expected result shape, and the forwarded workspace skills directory path (`workspace_skills_path` set to `.agents/skills/`, checking for primary `.agents/skills/` first and falling back to legacy `.agent/skills/` if the primary is missing). Prefer the same workspace for normal SDD phase execution; use an isolated Git worktree only when the user explicitly approves parallel write isolation.
